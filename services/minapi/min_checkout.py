...
def checkout(tier: str = "pro"):
    ...
    if not sec: raise HTTPException(500,"Stripe key missing")
    stripe.api_key=sec
    price=os.getenv("STRIPE_PRICE_PRO") if tier=="pro" else os.getenv("STRIPE_PRICE_BUSINESS")
    if not price: raise HTTPException(400,"price id missing")
    ...
    session=stripe.checkout.Session.create(mode="subscription", line_items=[{"price":price,"quantity":1}], success_url=success+"?session_id={CHECKOUT_SESSION_ID}", cancel_url=cancel, automatic_tax={"enabled":True}, allow_promotion_codes=True)
